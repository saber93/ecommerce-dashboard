...
The Last piece section appears only for a real, active product with exactly one sellable unit. Zero available units are omitted from the public catalog and return after restocking or an eligible order cancellation. The current six generated-photo products remain at 20 units each, so the live section is hidden. Open storefront tabs recheck catalog availability every minute while visible and on tab return; checkout still validates stock transactionally.
