...
Follow [deployment instructions](docs/deployment.md). Nine additive migrations, the six existing products, seven confirmed UAE delivery areas and Edge Function `commerce` version 10 are deployed to the existing cloud project. The storefront and separate Admin are deployed as development previews on Vercel. The owner reports adding the Admin Auth redirect URL; a read-only database check confirmed the requested administrator's verified email, password hash and private membership. Browser sign-in and authenticated Admin operations remain unverified. Cash on delivery remains closed while real photos and business details are missing. No real Ziina payment has been performed.
